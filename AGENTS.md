...
- Phase 2 每日循环：按 `DAILY-LOOP-SPEC.md` 做 Day 日志、健身输入、review mock。
- P2.3 真 AI：按 `P2.3-SPEC.md` 增加 Cloudflare Worker 代理；DeepSeek key 只进 Worker 加密变量，不进前端 / 仓库。
...
- [x] 大脑上方已加入空文字框；大脑节点改为星星，只有点击星星节点才弹出匿名卡片，旧卡片名称不再展示。
- [x] 当前公开 UI 收敛为纯大脑展示：左上角仅 `brain two`，中间只保留动态 3D 大脑；卡片库、今日区块、弹卡层、抽屉和命令面板都不在主页 UI 展示。
- [x] `P2.3-SPEC.md` 已入仓库，作为 P2.3 唯一规格。
- [x] `braintwo.zip` 内 20 张业务卡已并入默认业务库；旧 3 张样卡的本地缓存会自动迁移为 20 张真卡。
- [x] P2.3 代码已完成到可部署前状态：Cloudflare Worker `/review` + `/decide`、前端可配置 endpoint、每日复盘可切真代理、决策助手入口、A 的 promotedCards 本地写卡。
- [ ] P2.3 真 AI 验收待用户操作：Cloudflare 授权、填入 Worker 加密变量 `DEEPSEEK_API_KEY`、部署 Worker、在页面填 endpoint。
- [ ] P2.4 卡片提炼和夜间自动复盘暂缓。
